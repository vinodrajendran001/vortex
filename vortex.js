	...
	*/
		...
		for (var item in u[0][2])
		{
			if (zz<u[0][2][item]) {break;} //z_index is not number?
			z_index=z_index+1;
			z0=u[0][2][z_index];
		}
	...
	//141<=x<=147,35<=y<=43,0<=z_index<=30,0.5[0]<zz<820[30]
	    ...
	    //console.log("DEBUG");
		k2[0]=(threedinterpolate((x0+k1[0]*deltaT/2),
						(y0+k1[1]*deltaT/2),(z0+k1[2]*deltaT/2)))[0];
		k2[1]=(threedinterpolate((x0+k1[0]*deltaT/2),
						(y0+k1[1]*deltaT/2),(z0+k1[2]*deltaT/2)))[1];
		k2[2]=(threedinterpolate((x0+k1[0]*deltaT/2),
						(y0+k1[1]*deltaT/2),(z0+k1[2]*deltaT/2)))[2];
					
	
		k3[0]=(threedinterpolate((x0+k2[0]*deltaT/2),
						(y0+k2[1]*deltaT/2),(z0+k2[2]*deltaT/2)))[0];
		k3[1]=(threedinterpolate((x0+k2[0]*deltaT/2),
						(y0+k2[1]*deltaT/2),(z0+k2[2]*deltaT/2)))[1];
		k3[2]=(threedinterpolate((x0+k2[0]*deltaT/2),
						(y0+k2[1]*deltaT/2),(z0+k2[2]*deltaT/2)))[2];

		k4[0]=(threedinterpolate((x0+k3[0]*deltaT),
						(y0+k3[1]*deltaT),(z0+k3[2]*deltaT)))[0];
		k4[1]=(threedinterpolate((x0+k3[0]*deltaT),
						(y0+k3[1]*deltaT),(z0+k3[2]*deltaT)))[1];
		k4[2]=(threedinterpolate((x0+k3[0]*deltaT),
						(y0+k3[1]*deltaT),(z0+k3[2]*deltaT)))[2];
	    ...
	    console.log(pointsofvor[4000]);
	     ...
	     console.log(threedstreamline(pointsofvor[4000][0], 
	     pointsofvor[4000][1], pointsofvor[4000][2]-1,1000000, 0.02));//takes time to compute 10^6
